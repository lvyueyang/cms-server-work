...
export const getListApi = (body: SystemConfigQueryListDto) => {
	return request.post<SystemConfigListResponseDto>(
		`${AIP_FIX}/system_config/list`,
		body,
	);
};
...
export const getDetailApi = (id: number) => {
	return request.post<SystemConfigDetailResponseDto>(
		`${AIP_FIX}/system_config/info`,
		{ id },
	);
};
...
export const removeApi = (id: number) => {
	return request.post<Result<number>>(`${AIP_FIX}/system_config/delete`, {
		id,
	});
};
