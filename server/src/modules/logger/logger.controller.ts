...
import { LoggerByDateParamDto, LoggerDetailResponseDto, LoggerListResponseDto, LoggerFileParamDto } from './logger.dto';
...
export class LoggerController {
  ...
  async apiList() {
    ...
    const list = readdirSync(this.logDir).filter((f) => /^\d{4}-\d{2}-\d{2}$/.test(f));
    list.sort((a, b) => {
