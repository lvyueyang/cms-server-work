...
import { Form, message, ModalProps, Upload, Modal, Flex, Button } from 'antd';
  ...
  return (
    <div>
      ...
      <Form.Item label="JSON数据" required layout="vertical">
        <CodeEditor style={{ height: 400 }} value={importData} onChange={setImportData} />
      </Form.Item>
...
export function openImportJsonModal({
  ...
  onOk: (importData: string) => Promise<void>;
  help?: ReactNode;
}) {
  let importData = '';
  const m = modal.info({
    ...
    footer: (
      <Flex justify="flex-end" gap="16px 16px" style={{ marginTop: 24 }}>
