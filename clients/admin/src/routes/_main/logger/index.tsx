...
import { getDownloadUrl, getFileContentApi, getFilesApi } from "./module";
...
export default function LoggerDetail({ date }: DetailProps) {
	...
	const {
		...
		(filename) =>
			getFileContentApi(date, filename).then((res) => res.data.data),
		{
			manual: true,
		},
	);
...
export const Route = createFileRoute("/_main/logger/")({
