...
export class AuthService {
	...
	// 校验管理员用户是否登录
	async userAdminIsLogin({
		token,
	}: {
		token?: string;
	}): Promise<[UserAdmin | null, string | null]> {
		if (!token) {
		...
		try {
			...
			const user = await this.userAdminService.findOneById(
				tokenData.id as number,
			);
			// 退出登陆逻辑校验
	...
	// 校验客户端用户是否登录
	async userClientIsLogin({
		token,
	}: {
		token?: string;
	}): Promise<[UserClient | null, string | null]> {
		if (!token) {
			return [null, "token 不存在"];
		}
		try {
	...
	async validateAdminJwt(token: string) {
		...
		) as unknown as UserJWTPayload & jwt.JwtPayload;
	}
