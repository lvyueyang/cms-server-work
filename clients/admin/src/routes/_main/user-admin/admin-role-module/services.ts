...
export const getCodeListApi = () => {
	return request.get<AdminPermissionCodeListResponseDto>(
		`${AIP_FIX}/role/codes`,
		{},
	);
};
...
export const updateCodeApi = (
	id: number,
	body: AdminRoleUpdatePermissionCodeDto,
) => {
	return request.put<AdminRoleIdResponseDto>(
		`${AIP_FIX}/role/${id}/codes`,
		body,
	);
};
