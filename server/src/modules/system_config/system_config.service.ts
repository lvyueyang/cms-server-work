...
export class SystemConfigService {
	...
	findList(
		{ keyword = "", ...params }: CRUDQuery<SystemConfig>,
		lang?: ContentLang,
	) {
	...
	async findByCodes(codes: string[], lang?: ContentLang) {
		...
		if (lang && !isDefaultI18nLang(lang)) {
			const result = await this.contentTranslationService.overlayTranslations(
				list,
				{
					entity: SystemConfigService.I18N_KEY,
					fields: SystemConfigService.I18N_FIELDS,
	...
	async findNextAndPrev(id: number, lang?: ContentLang) {
		...
		if (lang && !isDefaultI18nLang(lang)) {
			const items = [nextInfo, prevInfo].filter((item): item is SystemConfig =>
				Boolean(item),
			);
			if (items.length) {
				const patched =
