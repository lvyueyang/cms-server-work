...
import { TrackEventSelect } from "@/routes/_main/track/meta/event-module/Select";
import { transformPagination } from "@/utils";
import { getListApi } from "./event-module";
	...
	const columns: ProColumns<TableItem>[] = [
		...
		},
		{
			...
			renderFormItem() {
				return (
					<Select
						options={propertiesKeyOptions.map((o: any) => ({
							label: o.cname,
							value: o.name,
						}))}
						allowClear
	...
	return (
		<>
			<PageTable<TableItem>
				columns={columns}
				request={(params) => {
					return getListApi({ ...params, ...transformPagination(params) }).then(
						({ data }) => {
...
export const Route = createFileRoute("/_main/track/event")({
