...
function BindEmail({ oldEmail, onSuccess }: BindEmailProps) {
	...
	return (
		<Form labelCol={{ flex: "70px" }} form={form}>
			<Divider>旧邮箱</Divider>
			<Form.Item label="旧邮箱">
				<div>{oldEmail}</div>
			</Form.Item>
			...
			<Form.Item dependencies={["new_email"]} label="验证码" required>
				{({ getFieldValue }) => (
					<Form.Item noStyle name="new_email_code" rules={[{ required: true }]}>
						<ValidateCodeInput
...
export const Route = createFileRoute("/_main/userinfo/")({
