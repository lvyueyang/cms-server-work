...
import type { PublicArticleCreateDto, PublicArticleUpdateDto } from '@cms/api-interface';
...
function PublicArticleFormPage() {
  ...
  const { run: submitHandler, loading } = useRequest(
    async () => {
      ...
      if (isUpdate) {
        ...
        navigate({ to: '/public-article/list' });
...
export const Route = createFileRoute('/_main/public-article/update/$id')({
