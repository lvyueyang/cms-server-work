...
import {
	...
	UserAdminHasRootUserResponseDto,
...
export class UserAdminController {
	...
	async update(
		@Body() { id, cname, avatar }: UserAdminUpdateDto,
	...
	@AdminLogin()
	async bindEmail(
		@Body() body: UserAdminBindEmailDto,
		@UserByAdmin() user: UserAdminInfo,
	) {
	...
	@Get("/has-root-user-admin")
	@ApiOperation({ summary: "检查是否已存在超级管理员用户" })
	@ApiOkResponse({
		type: UserAdminHasRootUserResponseDto,
	})
	async hasRootUser() {
		const hasRootUser = await this.services.hasRootUser();
		return successResponse(hasRootUser);
	}
