...
export function resolveDatabasePath(configuredPath?: string): string {
  ...
  const sqlitePath = isAbsolute(rawPath) ? rawPath : resolve(process.cwd(), rawPath);
 ...
 */
export function isDevSynchronizeEnabled(nodeEnv?: string, explicitFlag?: string): boolean {
