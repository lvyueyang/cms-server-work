...
export class WebhookTransService {
	...
	async send(key: string, data: any) {
		...
		try {
			...
			// 前置钩子判断
			const beforeHook = createMaxSafeFunction(
				`return ${info.before_hook_func}`,
			)(data);
			...
			// 数据处理
			const transFunc = createMaxSafeFunction(
				`return ${info.data_trans_func}`,
			)();
