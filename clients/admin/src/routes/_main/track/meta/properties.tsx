...
import {
	META_PROPERTIES_TYPE,
	META_PROPERTIES_TYPE_MAP,
} from "@cms/server/const";
...
export default function TrackMetaPropertiesListPage() {
	...
	const columns: ProColumns<TableItem>[] = [
		...
		},
		{
			...
			width: 90,
			render: (_, row) => {
				return META_PROPERTIES_TYPE_MAP.get(row.type as META_PROPERTIES_TYPE)
					?.label;
			},
			search: false,
		},
	...
	const formModal = useFormModal<
		TrackMetaPropertiesCreateDto & { id?: number }
	>({
		submit: (values, modal) => {
			...
			>
				...
				>
					...
					<Form.Item name="type" label="类型" rules={[{ required: true }]}>
						<Select
							options={enumMapToOptions(META_PROPERTIES_TYPE_MAP)}
							style={{ width: 160 }}
						/>
					</Form.Item>
...
export const Route = createFileRoute("/_main/track/meta/properties")({
	component: TrackMetaPropertiesListPage,
});
