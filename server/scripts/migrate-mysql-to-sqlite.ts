...
function parseArgs(argv: string[]): Args {
	...
	const sqlitePath = isAbsolute(configuredPath)
		? configuredPath
...
async function main() {
	...
	if (fileExistsNonEmpty(args.sqlitePath) && !args.force) {
		throw new Error(
			`目标 SQLite 已存在且非空: ${args.sqlitePath}。如需覆盖请加 --force。`,
		);
	}
	...
	console.log(
		`[init] mysql=${args.mysql.username}@${args.mysql.host}:${args.mysql.port}/${args.mysql.database}`,
	);
	...
	console.log(
		`[summary] copied=${copiedTotal} tables=${results.length} skipped=${skippedTotal}`,
	);
