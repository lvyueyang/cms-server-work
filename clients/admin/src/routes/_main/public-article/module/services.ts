...
export const getListApi = (body: PublicArticleQueryListDto) => {
	return request.post<PublicArticleListResponseDto>(
		`${AIP_FIX}/public_article/list`,
		body,
	);
};
...
export const getDetailApi = (id: number) => {
	return request.post<PublicArticleDetailResponseDto>(
		`${AIP_FIX}/public_article/info`,
		{ id },
	);
};
...
export const removeApi = (id: number) => {
	return request.post<Result<number>>(`${AIP_FIX}/public_article/delete`, {
		id,
	});
};
