...
function getMatchedRootRoute(pathname: string) {
	return adminMenuConfig.find((route) => {
		...
		return route.routes?.some((childRoute) =>
			matchRoutePath(childRoute.path, pathname),
		);
	});
}

export function getMenuViewByPathname(
	pathname: string = location.pathname,
): AdminMenuView {
	const hiddenRouteRule = adminMenuViewRules.find((rule) =>
		matchRoutePath(rule.path, pathname),
	);
	if (hiddenRouteRule) {
...
function getFirstVisiblePath(
	routers: AdminMenuNode[],
): AppNavigableRoutePath | undefined {
...
export function getMenuEntryPath(view: AdminMenuView): AppNavigableRoutePath {
	const entryPath = getFirstVisiblePath(
		adminMenuConfig.filter(
			(route) => getMenuGroup(route).key === getGroupKeyByView(view),
		),
	);
	...
	const fallbackView: AdminMenuView =
		view === "business" ? "platform" : "business";
	const fallbackPath = getFirstVisiblePath(
		adminMenuConfig.filter(
			(route) => getMenuGroup(route).key === getGroupKeyByView(fallbackView),
		),
	);
	if (fallbackPath) {
		console.warn(
			`[menu] Missing visible menu entry for ${view}, fallback to ${fallbackView}: ${fallbackPath}`,
		);
		return fallbackPath;
	}
...
function findMatchedMenuKeys(
	...
	for (const route of routers) {
		const visibleKeys = !route.menuHide
			? [...parentKeys, route.path]
			: parentKeys;
...
export function getMenuState(
	...
	const matchedRootRoute = adminMenuConfig.find(
		(route) => route.path === matchedKeys[0],
	);
