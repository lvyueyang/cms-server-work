...
import {
	...
	transEntityId,
} from "./content_translation.service";
...
export class ContentTranslationController {
	...
	@AdminRoleGuard(
		createPerm("admin:content_translation:upsert", `新增或更新${MODULE_NAME}`),
	)
	...
	@Post("/list")
	@AdminRoleGuard(
		createPerm("admin:content_translation:list", `获取${MODULE_NAME}列表`),
	)
	...
	@AdminRoleGuard(
		createPerm(
			"admin:content_translation:multi_upsert",
			`批量新增或更新${MODULE_NAME}`,
		),
	)
	...
	@Post("/update/byid")
	@AdminRoleGuard(
		createPerm("admin:content_translation:update", `更新${MODULE_NAME}`),
	)
	@ApiOkResponse({ type: ResponseResult<null> })
	...
	@Post("/delete")
	@AdminRoleGuard(
		createPerm("admin:content_translation:delete", `删除${MODULE_NAME}`),
	)
	@ApiOkResponse({ type: ResponseResult<null> })
