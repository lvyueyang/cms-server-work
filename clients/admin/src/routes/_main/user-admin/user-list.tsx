...
import { UserAdminCreateDto, UserAdminInfo, UserAdminUpdateDto, UserAdminUpdatePasswordDto } from '@cms/api-interface';
...
export default function UserAdminList() {
  ...
  const { submitLoading, form, formModal, formModalShow, formModalClose, submitHandler, formModalTitle } =
    useFormModal<UserAdminInfo>({
      submit: (values, modal) => {
        if (modal.type === ModalType.UPDATE && values.id) {
          ...
          } satisfies UserAdminUpdateDto).then(() => {
        ...
        if (modal.type === ModalType.OTHER && values.id) {
          ...
          } satisfies UserAdminUpdatePasswordDto).then(() => {
            tableRef.current?.reload();
          });
        }
        return createUser({
          ...
          cname: values.cname,
          email: values.email,
        } satisfies UserAdminCreateDto).then(() => {
          tableRef.current?.reload();
        });
      },
    });
  ...
  const submitRoleHandler = (roles: number[]) => {
    ...
    updateRole({ id: roleModal.user, roles })
      .then(() => {
      ...
      >
        ...
        >
          ...
          >
            <Input
              type="email"
              disabled={formModal.type !== ModalType.CREATE}
            />
          </Form.Item>
          <Form.Item
            name="cname"
...
export const Route = createFileRoute('/_main/user-admin/user-list')({
