...
export class TrackMetaEventService {
	...
	findList({
		...
		CRUDQuery<TrackMetaEvent> & { name?: string; cname?: string }) {
	...
	async create(data: FormValues, author: UserAdminInfo) {
		...
		const properties = await this.metaPropertiesServices.findByNames(
			data.properties,
		);
		...
		for (const preset of TRACK_META_EVENTS) {
			...
			if (exists) {
				...
				try {
					...
					console.warn(
						`Failed to create track meta property ${preset.name}:`,
						e,
					);
				}
			}
