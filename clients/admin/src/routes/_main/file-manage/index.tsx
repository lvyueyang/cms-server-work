...
import { createI18nColumn, formatFileSize, transformPagination, transformSort } from '@/utils';
...
const i18nColumn = createI18nColumn<TableItem>('file_manage');
  ...
  const columns: ProColumns<TableItem>[] = [
    ...
    },
    {
      ...
      render: (_, row) => {
        return (
          <AvailableSwitch
            value={row.login_download_auth || false}
            tableRef={tableRef}
            checkedChildren="是"
            unCheckedChildren="否"
            title={(isChecked) => `要${isChecked ? '关闭' : '开启'}登录后下载吗？`}
            request={() =>
              updateApi({
                id: row.id,
                login_download_auth: !row.login_download_auth,
              })
            }
          />
        );
...
export const Route = createFileRoute('/_main/file-manage/')({
