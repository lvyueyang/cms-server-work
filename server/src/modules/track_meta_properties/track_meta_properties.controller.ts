...
export class TrackMetaPropertiesController {
	...
	@AdminRoleGuard(
		createPerm("admin:track_meta_properties:list", `获取${MODULE_NAME}列表`),
	)
	...
	@AdminRoleGuard(
		createPerm("admin:track_meta_properties:info", `获取${MODULE_NAME}详情`),
	)
	...
	@AdminRoleGuard(
		createPerm("admin:track_meta_properties:create", `创建${MODULE_NAME}`),
	)
	async apiCreate(
		@Body() data: TrackMetaPropertiesCreateDto,
		@UserByAdmin() user: UserAdmin,
	) {
	...
	@AdminRoleGuard(
		createPerm("admin:track_meta_properties:update", `更新${MODULE_NAME}`),
	)
	...
	@AdminRoleGuard(
		createPerm("admin:track_meta_properties:delete", `删除${MODULE_NAME}`),
	)
